...
class ValidationTestCase(unittest.TestCase):
    def test_validate_invalid_request(self):
        ...
        self.assertEqual(serializer.data, {"username": "John Doe", "balance": 123 * 100})

    def test_validate_invalid_request_negative_balance(self):
        protobuf = Account(username="Jo", balance=-123)
        serializer = AccountSerializer(data=protobuf)
        self.assertEqual(serializer.is_valid(), False)
        self.assertEqual(
            serializer.errors,
            {"balance": "Balance must be positive", "username": "Username must be at least 3 characters long"},
        )
